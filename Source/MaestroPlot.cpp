...
#pragma omp parallel
        ...
        for (MFIter mfi(vel_mf, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            ...
            ParallelFor(tileBox, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                ...
                if (i == ilo && (phys_bc[0] == amrex::PhysBCType::inflow || phys_bc[0] == amrex::PhysBCType::slipwall ||
                                 phys_bc[0] == amrex::PhysBCType::noslipwall)) {
                    ...
                    uy = 0.5 * (u(i, j + 1, k, 0) - u(i, j - 1, k, 0)) / hy;
                           ...
                           (phys_bc[AMREX_SPACEDIM] == amrex::PhysBCType::inflow ||
                            phys_bc[AMREX_SPACEDIM] == amrex::PhysBCType::slipwall ||
                            phys_bc[AMREX_SPACEDIM] == amrex::PhysBCType::noslipwall)) {
                    vx = -(u(i - 1, j, k, 1) + 3.0 * u(i, j, k, 1) -
                           4.0 * u(i + 1, j, k, 1)) /
                         hx;
                    uy = 0.5 * (u(i, j + 1, k, 0) - u(i, j - 1, k, 0)) / hy;
                ...
                if (j == jlo && (phys_bc[1] == amrex::PhysBCType::inflow || phys_bc[1] == amrex::PhysBCType::slipwall ||
                                 phys_bc[1] == amrex::PhysBCType::noslipwall)) {
                    ...
                    uy = (u(i, j + 1, k, 0) + 3.0 * u(i, j, k, 0) -
                         ...
                         hy;
                           ...
                           (phys_bc[AMREX_SPACEDIM + 1] == amrex::PhysBCType::inflow ||
                            phys_bc[AMREX_SPACEDIM + 1] == amrex::PhysBCType::slipwall ||
                            phys_bc[AMREX_SPACEDIM + 1] == amrex::PhysBCType::noslipwall)) {
            ...
            ParallelFor(tileBox, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                ...
                bool fix_lo_x =
                    (phys_bc[0] == amrex::PhysBCType::inflow || phys_bc[0] == amrex::PhysBCType::noslipwall);
                bool fix_hi_x = (phys_bc[AMREX_SPACEDIM] == amrex::PhysBCType::inflow ||
                                 phys_bc[AMREX_SPACEDIM] == amrex::PhysBCType::noslipwall);

                bool fix_lo_y =
                    (phys_bc[1] == amrex::PhysBCType::inflow || phys_bc[1] == amrex::PhysBCType::noslipwall);
                bool fix_hi_y = (phys_bc[AMREX_SPACEDIM + 1] == amrex::PhysBCType::inflow ||
                                 phys_bc[AMREX_SPACEDIM + 1] == amrex::PhysBCType::noslipwall);

                bool fix_lo_z =
                    (phys_bc[2] == amrex::PhysBCType::inflow || phys_bc[2] == amrex::PhysBCType::noslipwall);
                bool fix_hi_z = (phys_bc[AMREX_SPACEDIM + 2] == amrex::PhysBCType::inflow ||
                                 phys_bc[AMREX_SPACEDIM + 2] == amrex::PhysBCType::noslipwall);
