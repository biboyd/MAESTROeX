    ...
    for (int lev = start_lev; lev <= end_lev; ++lev) {
        ...
        for (MFIter mfi(sold_mf, false); mfi.isValid(); ++mfi) {
            ...
            ParallelFor(xbx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                // lo x-faces
                if (i == domlo[0] - 1) {
                    switch (physbc[0]) {  // NOLINT(bugprone-switch-missing-default-case)
                ...
                // hi x-faces
                if (i == domhi[0] + 2) {
                    switch (physbc[AMREX_SPACEDIM]) {  // NOLINT(bugprone-switch-missing-default-case)
                        case amrex::PhysBCType::inflow:
                            umac(i, j, k) = umac(i - 1, j, k);
                            vmac(i - 1, j, k) = 0.0;
#if (AMREX_SPACEDIM == 3)
            ...
            ParallelFor(ybx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                // lo y-faces
                if (j == domlo[1] - 1) {
                    switch (physbc[1]) {  // NOLINT(bugprone-switch-missing-default-case)
                        case amrex::PhysBCType::inflow:
                            umac(i, j, k) = 0.0;
                ...
                // hi y-faces
                if (j == domhi[1] + 2) {
                    switch (physbc[AMREX_SPACEDIM + 1]) {  // NOLINT(bugprone-switch-missing-default-case)
                        case amrex::PhysBCType::inflow:
                            umac(i, j - 1, k) = 0.0;
                            vmac(i, j, k) = vmac(i, j - 1, k);
#if (AMREX_SPACEDIM == 3)
            ...
            ParallelFor(zbx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                // lo z-faces
                if (k == domlo[2] - 1) {
                    switch (physbc[2]) {  // NOLINT(bugprone-switch-missing-default-case)
                        case amrex::PhysBCType::inflow:
                            umac(i, j, k) = 0.0;
                            vmac(i, j, k) = 0.0;
                ...
                // hi z-faces
                if (k == domhi[2] + 2) {
                    switch (physbc[2 + AMREX_SPACEDIM]) {  // NOLINT(bugprone-switch-missing-default-case)
                        case amrex::PhysBCType::inflow:
                            umac(i, j, k - 1) = 0.0;
                            vmac(i, j, k - 1) = 0.0;
                            wmac(i, j, k) = wmac(i, j, k - 1);
